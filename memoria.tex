...
\begin{table}[htb]  % [htb] suggests table position in page
 ...
 \begin{center}
  ...
  % \begin{tabular} { | l | c | r |} % tenemos tres colummnas, la primera alineada a la izquierda (l), la segunda al centro (c) y la tercera a la derecha (r). Se elimina el símbolo |, que indica que entre las columnas habría una línea separadora.
    ...
    \textbf{Human} & FP: 13 & TN: 1776 & 0.99 & 0.99 & 0.99 \\ %\hline
...
% {\color{red} TODO: Add schema of the cycle of data science.} \\
...
% given training vectors \( x_i \in R^n, i=1,…, l\) and a label vector $y \in R^l$, a decision tree recursively partitions the feature space such that the samples with the same labels or similar target values are grouped together %METER CITA%.

% Let the data at node $m$ be represented by $Q_{m}$ with $n_{m}$ samples. For each candidate split $\theta = (j, t_{m})$ consisting of a feature $j$ and threshold $t_{m}$, partition the data into $Q_m^{left}(\theta)$ and $Q_m^{right}(\theta)$ subsets:
...
% \begin{center}
% \begin{equation}
% \theta^* = \operatorname{argmin}_\theta  G(Q_m, \theta)\quad ,
% \end{equation}
% \end{center}
% \smallskip

% and recurse for subsets $Q_m^{left}(\theta^*)$ and $Q_m^{right}(\theta^*)$ until the maximum allowable depth is reached, $n_{m} < min_{samples}$ or $n_{m} = 1$.
...
% \[
% d_{M}({\vec {x}},Q)={\sqrt {({\vec {x}}-{\vec {\mu }})^{\mathsf {T}}S^{-1}({\vec {x}}-{\vec {\mu }})}}
% \]
...
% The piece of code used to try to compute the Malahalobis distances is available on listing \ref{lst:mahalanobis}. As some of the input matrices were singular (they had no inverse matrix), the code raised the following error: \texttt{LinAlgError: Singular matrix}.
...
% Then, the result is normalized between 0 and 100.
...
% \begin{center}
%     \begin{math}
%         \mathrm {Ts}(term) = 100* \frac{(60*Nl_{1} + 30*Nl_{2} + 10*Nl_{3})}{n(l_{1}) + n(l_{2}) + n(l_{3})} 
%     \end{math}
% \end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \chapter{Metrics} (to be deprecated).
% \label{app:app-metrics}
% For now, this is an initial list of metrics by channel.

% \section{Git (commits), per individual}
...
% \section{GitHub Issues, per individual}
...
%     \item \textbf{M15}: Frequency of issue creation (define base period of time)
...
% \section{GitHub Pull Requests, per individual}
...
%     \item \textbf{M28}: Number of distinct reactions.
%     \item \textbf{M22}: Frequency of PRs creation (define base period of time)
